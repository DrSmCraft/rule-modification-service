...
public class ParamInjectorController {
    ...
    Logger logger = LoggerFactory.getLogger(ParamInjectorController.class);

    @Autowired
    ParamInjectorService injectorService = new ParamInjectorService();
    ...
    String injectAndTranslate(@RequestBody ParamInjectionRequestBody body, @RequestParam(required = false) Boolean ignoreErrors) throws UnsupportedEncodingException {
        checkParamInjectionRequestBody(body);
        if (ignoreErrors == null) {
            ignoreErrors = false;
        }
        ParamInjectionRequestBody injectedRequest = inject(body);
        return translate(injectedRequest, ignoreErrors, false, false, false, false, false, false, CqlCompilerException.ErrorSeverity.Error, false, false, false, false, false, false, false, false, false, LibraryBuilder.SignatureLevel.All, "1.5");
    }
    ...
    void checkParamInjectionRequestBody(ParamInjectionRequestBody body) {
